...
1.  Receives three arguments: `expo`, `cumadopt`, and `time`.
    ...
    a.  `expo` is the exposure *array* with three dimensions: \# of nodes $\times$ \# of time points $\times$ \# of behaviors.
    b.  `cumadopt` is the cumulative adoption *array*, with the **same dimensions** as `expo`. So, if `cumadopt[i, t, q] = 1`, then `i` is an adopter of behavior `q` at time `t`.
...
```
  ...
  create a disadoption function that randomly selects **10% of the adopters** at time `t - 1` for $q$ behaviors. ([solution file](files/sim-solutions-pt2-1.R))
